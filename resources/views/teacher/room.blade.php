...
@section('container')
    ...
    <div onclick="openPopup()" class="rounded-lg bg-lime-600 hover:bg-lime-700 p-2 w-min my-2 text-white cursor-pointer">Tambah</div>
    ...
    @foreach ($students as $student)
        <div class="py-1 border border-px border-pink-600">
            ...
            <h1 class="text-center">{{ $student->nis }}</h1>
        </div>
    @endforeach
    ...
    <div id="popup" class="hidden fixed bg-black top-0 left-0 z-50 w-full h-full flex justify-center items-center bg-opacity-60">
        <div class="rounded-md w-80 md:w-96 bg-white px-10 py-4 absolute">
            ...
            <form action="/siswa/tambah" method="POST">
                @csrf
                <input type="text" name="name" placeholder="Nama Siswa" class="w-full mb-4 border border-1 border-grey-200 p-2 rounded-lg" id="">
                <input type="text" name="arabic" placeholder="Nama Arab" class="w-full mb-4 border border-1 border-grey-200 p-2 rounded-lg" id="">
                <input type="text" name="nis" placeholder="NIS" class="w-full mb-4 border border-1 border-grey-200 p-2 rounded-lg" id="">
                ...
                <input type="text" class="hidden" value="{{ $students[0]->group_id }}" name="grup">
