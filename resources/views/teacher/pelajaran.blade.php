...
<div id="alert-1" class="flex items-center p-4 my-4 text-blue-800 rounded-lg bg-blue-50" role="alert">
    ...
    <button type="button" class="ms-auto -mx-1.5 -my-1.5 bg-blue-50 text-lime-600 rounded-lg focus:ring-2 focus:ring-blue-400 p-1.5 hover:bg-blue-200 inline-flex items-center justify-center h-8 w-8 dark:hover:bg-gray-700" data-dismiss-target="#alert-1" aria-label="Close">
    ...
    <thead class="text-xs text-white uppercase bg-pink-600">
        <tr>
            ...
            <tbody>
                ...
                <tr class="bg-white border-1 border-grey-900 dark:bg-gray-800 dark:border-gray-700">
                    <td class="px-6 py-4">
                        @if ($room->course==0)
                        <a class="text-red-600" href="/pelajaran/hapus/kelas/{{ $course->id }}"><img class="inline" src="{{ Storage::url('assets/trash.svg') }}" alt=""> </a>
                    ...
                    <td class="px-6 py-4 text-center">
                        ...
                        <form action="/pelajaran/ubah/kelas" method="POST">
                            ...
                            <input type="text" class="w-10 text-center rounded-lg bg-slate-300" maxlength="2" name="kkm" value="{{ $course->kkm }}" id="">
                            <input type="submit" value="Ubah" class="hover:text-white hover:bg-pink-600 font-semibold text-xs p-1 rounded-md">
                        </form>
                        @else
                        {{ $course->kkm }}                        
                        @endif
        ...
        <div class="rounded-md w-96 bg-white px-10 py-4 absolute">
            ...
            <form action="/pelajaran/tambah/kelas" method="POST">
                @csrf
                <input type="text" class="hidden" name="class_code" value="{{ $room->class_code }}">
