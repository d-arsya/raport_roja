...
@section('container')
    <div onclick="openPopup()"
        class="text-white bg-pink-600 hover:bg-pink-700 w-max font-medium rounded-lg text-sm px-3 py-2 me-2 mb-2 cursor-pointer">
        <svg class="w-5 h-5 text-gray-800 inline" width="24" height="24" fill="none" viewBox="0 0 24 24">
    ...
    <div class="mt-3 grid grid-cols-1 sm:grid-cols-3 justify-align-center gap-3">
        @foreach ($rooms as $kelas)
            <span class="block max-w-full p-6 bg-white rounded-lg shadow-lg">
                ...
                @if ($kelas->teacher->name ?? false)
                    <form action="/kelas/edit/guru" onchange="onch(this)" method="post" class="max-w-sm mx-auto mt-3">
                        @csrf
                        ...
                        <select name="nip"
                            ...
                            @foreach ($teachers as $teacher)
                                <option value="{{ $teacher->nip }}"
                                    @if ($kelas->teacher->name == $teacher->name) {{ 'selected' }} @endif>
        ...
        <div class="rounded-md w-80 md:w-96 bg-white px-10 py-4 absolute">
            ...
            <form action="/kelas/tambah" method="POST">
                ...
                <input type="number" name="semester" placeholder="Semester"
                    class="w-full mb-4 border border-1 border-grey-200 p-2 rounded-lg" id="">
                <select name="nip" id="guru"
                    class="bg-gray-50 mb-8 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
